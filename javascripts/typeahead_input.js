 ...
 * @config {number} delay time in milliseconds to wait between the user entering a keystroke and the 'change' event firing
 *
...
jive.TypeaheadInput = jive.oo.Class.extend(function(protect) {
    ...
    protect.emitChange = function(value) {
        if ($.trim(value) === '' || value.length < this.minLength) {
            this.emit('clear');
